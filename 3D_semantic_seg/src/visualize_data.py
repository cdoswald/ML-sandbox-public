...
def process_scene(
    ...
    print(
        f"[PID {os.getpid()}] {file_id}: # of labeled obs ids: {len(labeled_obs_ids)}"
    )
    ...
    filter_camera_rows = {
        "index": labeled_obs_ids,
        "key.camera_name": list(np.arange(1, 6)),
    }
    ...
    for camera_id in range(10):
        ...
        try:
            ...
            print(
                f"[PID {os.getpid()}] {file_id}: Could not extract frames for camera_id={camera_id}; got error: '{e}'"
            )
    ...
    output_pointcloud_file = os.path.join(
        args.pointcloud_dir, f"pointcloud_{file_id}.h5"
    )
    with h5py.File(output_pointcloud_file, "w") as f:
        for labeled_obs_id in sorted(labeled_obs_ids):
            ...
            f.create_dataset(
                str(labeled_obs_id), data=points, compression="gzip", compression_opts=4
            )
    ...
    if args.scene_vis_vstack_videos:
        ...
        pointcloud_video_file = os.path.join(
            args.videos_dir, f"pointcloud_{file_id}.mp4"
        )
        if os.path.exists(camera_video_file) and os.path.exists(pointcloud_video_file):
            combined_video_file = os.path.join(
                args.videos_dir, f"combined_{file_id}.mp4"
            )
